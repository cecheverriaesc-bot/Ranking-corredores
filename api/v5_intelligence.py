...
def fetch_broker_phones_map():
    ...
    try:
        ...
        cursor.execute(
            "SELECT nombre, apellido, telefono, celular FROM corredores WHERE activo = 1"
        )
...
def calculate_dias_restantes(year=None, month=None):
    ...
    if not is_current_month(use_year, use_month):
        ...
        today = date.today()
        if use_year < today.year or (
            use_year == today.year and use_month < today.month
        ):
            return 0
...
def fetch_squad_intelligence_v5(
    coordinador_email="carlos.echeverria", filter_region="ALL", year=None, month=None
):
    """
    ...
    try:
        ...
        cursor.execute(
            "SELECT SUM(reserva) as total_global FROM bi_DimCorredores WHERE activo = 1"
        )
        total_global_hist = float(cursor.fetchone()["total_global"] or 1)

        cursor.execute(
            "SELECT SUM(reserva) as total_equipo FROM bi_DimCorredores WHERE coordinador = %s AND activo = 1",
            (coordinador_email,),
        )
        total_equipo_hist = float(cursor.fetchone()["total_equipo"] or 0)

        pct_equipo = (
            total_equipo_hist / total_global_hist if total_global_hist > 0 else 0
        )
        ...
        if month == 'all':
            cursor.execute("""
                SELECT COUNT(DISTINCT lead_id) as contratos_periodo
                FROM bi_DimLeads
        ...
        contratos_equipo_mes = int(cursor.fetchone()["contratos_periodo"] or 0)

        # ================================================================
        # PASO 3: Data Individual Base con Comunas, Métricas de Tiempo y Teléfono (desde Zendesk)
        # ================================================================
        debug_log.append(
            "Query 3: Data individual base con comunas, tiempos y telefono"
        )
        if month == 'all':
            time_filter = f"YEAR(fecha_tomado) = {use_year}"
            time_filter_agenda = f"YEAR(agenda_fecha) = {use_year}"
        else:
            time_filter = f"YEAR(fecha_tomado) = {use_year} AND MONTH(fecha_tomado) = {use_month}"
            time_filter_agenda = f"YEAR(agenda_fecha) = {use_year} AND MONTH(agenda_fecha) = {use_month}"
        ...
        cursor.execute(f"""
            SELECT
                ...
                COALESCE(l.contratos_periodo, 0) as contratos_mes,
                COALESCE(l.leads_tomados_periodo, 0) as leads_tomados_mes,
                COALESCE(l.prospectos_periodo, 0) as prospectos_mes,
            ...
            LEFT JOIN (
                SELECT
                    ...
                    GROUP_CONCAT(DISTINCT CASE WHEN comuna IS NOT NULL THEN comuna END SEPARATOR ',') as comunas
                FROM bi_DimLeads
                WHERE {time_filter}
            ...
            GROUP BY c.corredor_id, c.nombre_corredor, c.reserva,
                     l.contratos_periodo, l.leads_tomados_periodo, l.prospectos_periodo,
                     l.leads_descartados_sin_gestion, l.prospectos_descartados, l.contacto_24h,
                     l.accion_24h, l.comunas
        ...
        for corredor in corredores_data:
            ...
            pct_corredor = (
                reservas_hist / total_equipo_hist if total_equipo_hist > 0 else 0
            )
            ...
            tasa_visitas = calculate_rate_with_smoothing(
                visitas_realizadas, total_agendas, 15
            )
            tasa_cancelacion = calculate_rate_with_smoothing(
                visitas_canceladas, total_agendas, 15
            )
            tasa_descarte_leads = calculate_rate_with_smoothing(
                leads_descartados, leads_mes, 15
            )
            tasa_descarte_prospectos = calculate_rate_with_smoothing(
                prospectos_descartados, prospectos_mes, 15
            )
            ...
            conv_prospecto_contrato = calculate_rate_with_smoothing(
                contratos_mes, prospectos_mes, 15
            )
            conv_lead_contrato = calculate_rate_with_smoothing(
                contratos_mes, leads_mes, 15
            )
            # Capacidad productiva (valores absolutos y tasas)
            contratos_absolutos = contratos_mes  # Valor absoluto
            leads_por_visita = (
                calculate_rate_with_smoothing(leads_mes, visitas_realizadas, 0)
            ...
            tasa_demora = calculate_rate_with_smoothing(
                prospectos_demora, prospectos_mes, 15
            )
            # Tiempo promedio de resolución (inverso - menor es mejor)
            # Normalizar a horas, asumir max 72h como peor caso
            tiempo_normalizado = (
                min(tiempo_prom_resolucion / 72, 1) if tiempo_prom_resolucion > 0 else 0
            )
            # Tickets severidad (inverso - menor es mejor)
            # Normalizar (asumir max 30 puntos como peor caso)
            tickets_normalizado = (
                min(tickets_severidad / 30, 1) if tickets_severidad > 0 else 0
            )
            ...
            leads_diarios_necesarios = (
                int((faltante / max(days_remaining, 1)) / (conv_mes / 100))
                if conv_mes > 0 and days_remaining > 0
                else 0
            )
            ...
            broker_data = {
                ...
                "telefono": phones_map.get(
                    corredor["nombre_corredor"]
                ),  # Teléfono real desde Rentas
        ...
        all_tasa_cancelacion = [
            b["tasa_cancelacion"] for b in brokers_to_score
        ]  # Inversa
        all_tasa_descarte_leads = [
            b["tasa_descarte_leads"] for b in brokers_to_score
        ]  # Inversa
        all_tasa_descarte_prospectos = [
            b["tasa_descarte_prospectos"] for b in brokers_to_score
        ]  # Inversa
        ...
        all_tickets_norm = [
            b["tickets_normalizado"] for b in brokers_to_score
        ]  # Inverso
        ...
        norm_tasa_cancelacion = normalize_z_score_robust(
            all_tasa_cancelacion, inverse=True
        )
        norm_tasa_descarte_leads = normalize_z_score_robust(
            all_tasa_descarte_leads, inverse=True
        )
        norm_tasa_descarte_prospectos = normalize_z_score_robust(
            all_tasa_descarte_prospectos, inverse=True
        )
        norm_tasa_accion_24h = normalize_z_score_robust(
            all_tasa_accion_24h, inverse=False
        )
        ...
        for i, broker in enumerate(brokers_to_score):
            # ============================================
            # PILAR 1: ENGAGEMENT & GESTIÓN ACTIVA (30%)
            # ============================================
            # % visitas realizadas / agendadas (20% del pilar = 6% del total)
            eng_visitas = norm_tasa_visitas[i] * 6
            # % visitas no canceladas (20% del pilar = 6% del total)
            eng_no_cancela = norm_tasa_cancelacion[i] * 6
            # % leads NO descartados (20% del pilar = 6% del total)
            eng_no_descarta_leads = norm_tasa_descarte_leads[i] * 6
            # % prospectos NO descartados (20% del pilar = 6% del total)
            eng_no_descarta_prospectos = norm_tasa_descarte_prospectos[i] * 6
            # % leads con acción 24h (20% del pilar = 6% del total)
            eng_accion_24h = norm_tasa_accion_24h[i] * 6

            engagement_score = (
                eng_visitas
                + eng_no_cancela
                + eng_no_descarta_leads
                + eng_no_descarta_prospectos
                + eng_accion_24h
            )

            # ============================================
            # PILAR 2: RENDIMIENTO & CONVERSIÓN (55%)
            # ============================================
            # Eficacia (50% del pilar = 27.5% del total)
            rend_conv_p_c = norm_conv_p_c[i] * 13.75  # % prospecto→contrato
            rend_conv_l_c = norm_conv_l_c[i] * 13.75  # % lead→contrato

            # Capacidad productiva (50% del pilar = 27.5% del total)
            rend_contratos_abs = norm_contratos_abs[i] * 13.75  # Contratos absolutos
            rend_lpv = norm_lpv[i] * 13.75  # Leads/visita

            rendimiento_score = (
                rend_conv_p_c + rend_conv_l_c + rend_contratos_abs + rend_lpv
            )

            # ============================================
            # PILAR 3: EFICIENCIA OPERATIVA Y TIEMPOS (15%)
            # ============================================
            # % prospectos SIN demora (33% del pilar = 5% del total)
            efi_demora = norm_tasa_demora[i] * 5
            # Tiempo de resolución (33% del pilar = 5% del total)
            efi_tiempo = norm_tiempo_norm[i] * 5
            # Tickets severidad (34% del pilar = 5% del total)
            efi_tickets = norm_tickets_norm[i] * 5
            ...
            brokers_final.append(
                {
                    ...
                    "comunas": broker["comunas"][:5]
                    if len(broker["comunas"]) > 5
                    else broker["comunas"],
                    ...
                    "breakdown_engagement": {
                        ...
                        "prospectos_no_descartados": round(
                            eng_no_descarta_prospectos, 2
                        ),
        ...
        return {
            ...
            "brokers_regiones": [
                b for b in brokers_final if b["region_type"] == "REGIONES"
            ],
            "leader": leader,
            "squad_summary": {
                "meta_equipo": META_EQUIPO,
...
class handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        try:
            ...
            data = fetch_squad_intelligence_v5(
                coordinator, region, year=year, month=month
            )
